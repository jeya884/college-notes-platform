...
function setupUploadForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            form.reset();
            setTimeout(() => loadManageNotes(), 2000);
        } catch (error) {
                ...
                <tbody>
                    ${notes.map(note => `
                        <tr style="border-bottom: 1px solid #e5e7eb;">
                            <td style="padding: 12px; border: 1px solid #e5e7eb;">${escapeHtml(note.title)}</td>
...
function setupManageUsers() {
    ...
    if (search) {
        search.addEventListener('keyup', loadManageUsers);
    }
}
            ...
            <tbody>
                ${users.map(user => `
                    <tr style="border-bottom: 1px solid #e5e7eb;">
                        ...
                        <td style="padding: 12px; border: 1px solid #e5e7eb;">${new Date(user.created_at).toLocaleDateString()}</td>
                    </tr>
                `).join('')}
            </tbody>
    try {
        ...
        downloads.innerHTML = topDownloads.slice(0, 5).map(note => 
            `<li>${escapeHtml(note.title)} - ${note.download_count} downloads</li>`
        ...
        ratings.innerHTML = topRated.slice(0, 5).map(note => 
            `<li>${escapeHtml(note.title)} - ⭐ ${(note.average_rating || 0).toFixed(1)}</li>`
        ).join('') || '<li>No data available</li>';
    } catch (error) {
        console.error('Error loading reports:', error);
        showNotification('Error loading reports', 'error');
    } = await fetchNotes({ sort: 'rating' });
    const topRated = rated.slice(0, 5);

    const ratings = document.getElementById('topRated');
    ratings.innerHTML = topRated.map(note => 
        `<li>${escapeHtml(note.title)} - ⭐ ${(note.average_rating || 0).toFixed(1)}</li>`
    ).join('');
}
